...
import { Bell, Camera, ChevronRight, Flame } from "lucide-react";
    ...
    <AppShell viewportLocked>
      <div className="relative flex h-full min-h-0 flex-col bg-[#f5f6f8] text-[#191f28]">
        <header
          ...
          className={`${styles.header} absolute inset-x-0 top-0 z-10 flex h-16 items-center justify-between bg-[#f7f8fa]/95 px-5 py-3 backdrop-blur`}
        >
          <Image
        ...
        >
          {error ? (
            <div
              role="alert"
              className="mb-3 flex items-center gap-3 rounded-xl bg-[#fff0f0] px-4 py-3 text-xs text-[#d91b34]"
            >
          ...
          <section className="h-[360px] overflow-hidden rounded-[20px] bg-white px-5 pt-6 pb-5 shadow-[0_2px_8px_rgba(26,33,48,0.06)]">
            <Link
              href="/home/streak"
              className="inline-flex min-h-11 items-center gap-1.5 rounded-full bg-[#eaf4ff] px-3 text-[11px] leading-[14px] font-bold tracking-[0.034em] text-primary"
            >
              <Flame className="size-4 fill-current" /> 오늘{" "}
              {today.completedCount}/{today.goalCount}회 완료
            </Link>
            ...
            <Link
              ...
              className="mt-[18px] flex h-14 w-full touch-manipulation items-center justify-center rounded-full bg-primary px-7 text-base leading-6 font-bold tracking-[0.0057em] text-white transition duration-150 active:scale-[0.985] active:bg-[#006bd6]"
            >
          ...
          <Link
            href="/lip-practice"
            className="mt-3 flex min-h-[112px] items-center gap-4 overflow-hidden rounded-[20px] bg-[#eaf4ff] p-5 shadow-[0_8px_24px_rgb(0_128_255_/_8%)]"
          >
            <span className="flex size-14 shrink-0 items-center justify-center rounded-[20px] bg-primary text-white">
              <Camera className="size-7" />
            </span>
            <span className="min-w-0 flex-1">
              <span className="block text-[15px] font-bold">입모양 연습</span>
              <span className="mt-1 block text-xs leading-5 text-[#4e5968]">
                영상을 기기에만 보관하며 5개 문장을 촬영해요
              </span>
            </span>
