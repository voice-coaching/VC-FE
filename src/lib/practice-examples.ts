...
export type PracticeExampleCategory =
  | "consonant"
  | "vowel"
  | "final-consonant"
  | "tense-consonant"
...
export interface ResolvedPracticeExample extends PracticeExample {
  category: PracticeExampleCategory;
  label: string;
  learningFocus: LearningFocus;
}

interface PracticeExampleSet {
  label: string;
  learningFocus: LearningFocus;
  examples: readonly PracticeExample[];
}
...
} as const satisfies Record<PracticeExampleCategory, PracticeExampleSet>;

export function inferPracticeExampleCategory({
  courseType,
  courseTitle,
  stepTitle,
}: {
  ...
  courseTitle: string;
  stepTitle: string;
}): PracticeExampleCategory {
  const label = `${courseTitle} ${stepTitle}`;
  if (/된소리|쌍자음/.test(label)) return "tense-consonant";
  if (/받침|겹받침|연음|유음|ㄹ/.test(label)) return "final-consonant";
  if (/모음/.test(label)) return "vowel";
  if (/자음/.test(label)) return "consonant";
  if (/의문|질문|올리/.test(label)) return "rising-intonation";
  if (/강조|핵심/.test(label)) return "word-stress";
  if (/평서|내리/.test(label)) return "falling-intonation";
  return courseType === "INTONATION" ? "falling-intonation" : "final-consonant";
}

export function getPracticeExampleSet(input: {
  courseType: CourseType;
  courseTitle: string;
  stepTitle: string;
}): PracticeExampleSet & { category: PracticeExampleCategory } {
  const category = inferPracticeExampleCategory(input);
  return { category, ...practiceExampleSets[category] };
}

export function findPracticeExample(
  exampleId: string | null | undefined,
): ResolvedPracticeExample | undefined {
  if (!exampleId) return undefined;
  for (const [category, set] of Object.entries(practiceExampleSets)) {
    const example = set.examples.find((item) => item.id === exampleId);
    if (example)
      return {
        ...example,
        category: category as PracticeExampleCategory,
        label: set.label,
        learningFocus: set.learningFocus,
      };
  }
  return undefined;
}

export function createExamplePracticeContent(
  ...
  return {
    ...
    title: `${example.label} 예시 문제`,
    category: example.label,
    difficulty,
    learningFocus: example.learningFocus,
    description: example.hint,
    scriptText: example.text,
    targetPronunciations: [example.category],
    estimatedSeconds: 30,
